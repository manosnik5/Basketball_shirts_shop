...
import { usePendingAddress } from "@/lib/hooks/usePendingAddress";
...
const PaymentPage = () => {
  ...
  // Authenticated users: load address by userId
  // Guests: load address by the pending_shipping_address_id cookie
  const {
    data: userAddressData,
    isLoading: isUserAddressLoading,
    isError: isUserAddressError,
  } = useGetAddress(userId, { enabled: !!userId });

  const {
    data: pendingAddressData,
    isLoading: isPendingAddressLoading,
    isError: isPendingAddressError,
  } = usePendingAddress({ enabled: !userId });

  const { mutate: createOrder, isPending } = useCreateOrder();

  const isAddressLoading = userId ? isUserAddressLoading : isPendingAddressLoading;
  const isAddressError = userId ? isUserAddressError : isPendingAddressError;
  const address = userId ? userAddressData?.address : pendingAddressData?.address;

  if (isAddressLoading) return <p>Loading address...</p>;
  if (isAddressError) return <p>Failed to load address</p>;
  if (!address) return <p>No address found. Please complete the shipping step first.</p>;
  ...
  const validateCreditCardFields = () => {
    ...
    if (cardNumberInput) {
      ...
      if (!cardValidation.isValid) errors.cardNumber = cardValidation.error!;
    }
    if (expiryInput) {
    ...
    if (cvvInput) {
      ...
      if (!cvvValidation.isValid) errors.cvv = cvvValidation.error!;
    }
  ...
  const handlePlaceOrder = () => {
    ...
    if (creditCardSelected && !validateCreditCardFields()) {
    ...
    createOrder(
      {
        // userId is optional — guests pass undefined
        ...(userId ? { userId } : {}),
        ...
        <div className="flex flex-col">
          ...
          <div className="border border-gray-300 px-6 py-2">
            <div className="flex flex-col gap-4 py-2">
              ...
              <label className="flex items-center gap-2">
                <input
                  type="radio" name="payment" value="klarna"
                  className="accent-red-600"
              ...
              <label className="flex items-center gap-2">
                <input
                  type="radio" name="payment" value="credit_card"
                  className="accent-red-600"
            ...
            <label className="flex items-center gap-2 mt-2">
              <input type="checkbox" name="sameAsShipping" className="accent-red-600" checked readOnly />
              Same as shipping address
            </label>
            <p className="text-sm text-gray-600 mt-2">
              By placing an order, you agree to our{" "}
              <a href="/terms" className="underline text-blue-600">Terms of Use</a>{" "}
              &{" "}
            ...
            return (
              <div key={item.id} className="flex justify-between flex-col-2">
                <Image
                  ...
                  width={100}
                  height={100}
                  className="object-cover rounded-md w-25 h-25 md:w-34 md:h-34"
                />
                <div className="space-y-2">
                  <p>{item.variant.shirt.name}</p>
                  <p className="text-dark font-medium">
                    {formatPrice(Number(item.variant.salePrice ?? item.variant.price) * item.quantity)}
                  </p>
